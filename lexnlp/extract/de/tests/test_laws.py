...
def setup_parser():
    ...
    gesetze_df = pd.read_csv(os.path.join(os.path.dirname(__file__), base_path + "gesetze_list.csv"), encoding="utf-8")

    verordnungen_df = pd.read_csv(
        os.path.join(os.path.dirname(__file__), base_path + "verordnungen_list.csv"), encoding="utf-8"
    )

    concept_df = pd.read_csv(
        os.path.join(os.path.dirname(__file__), base_path + "de_concept_sample.csv"), encoding="utf-8"
    )

    law_parser = LawsParser(gesetze_df, verordnungen_df, concept_df)
    return law_parser
...
class TestParseDeLaws(TestCase):
    ...
    def test_file_samples(self):
        ...
        tester.test_and_raise_errors(
            get_ordered_law_annotations, "lexnlp/typed_annotations/de/law/laws.txt", LawAnnotation
        )
