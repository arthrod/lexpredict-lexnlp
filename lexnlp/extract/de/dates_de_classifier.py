...
def train_default_model(save=True, verbose=False, check_date_strings=False):
    ...
    build_date_model(
        ...
        output_path,
        lambda date_str: [
            (d["value"].date(), (d["location_start"], d["location_end"])) for d in parser.get_dates(date_str)
        ],
...
def make_date_samples():
    examples = [
        ("""Spätestens am 01.06.2017""", [datetime.date(2017, 6, 1)]),
        ...
        ("""Gesamt 6.576.210,93 Darlehen       11.12.2007 10.000.000,00""", [datetime.date(2007, 12, 11)]),
        (
        ...
        ("""zuletzt geändert durch Art. 39 G v. 29.3.2017 I 626""", [datetime.date(2017, 3, 29)]),
        ...
        ("""Was passiert, wenn das Gebäude am 01.10.2015 verkauft wird""", [datetime.date(2015, 10, 1)]),
        ("""am siebzehnten Oktober eintausendneunhundertdreiundachtzig""", [datetime.date(1983, 10, 17)]),
