...
def refine_pattern_start_end(pattern: str) -> str:
    ...
    """
    return "|".join([rf"(?<![^\W\d_]){p}(?![^\W\d_])" for p in pattern.split("|")])
...
class DateFinder:
    ...
    DIGITS_MODIFIER_PATTERN = (
        r"\d+st|\d+th|\d+rd|first|second|third|fourth|fifth|sixth|seventh|eighth|nineth|tenth|next|last"
    )
    ...
    EXTRA_TOKENS_PATTERN = (
        "due|by|on|during|standard|daylight|savings|time|date|dated|day|of|from|to|through|between|until|z|at|t"
    )
    ...
    def extract_date_strings_inner(self, text: str, text_start: int = 0, strict=False):
        ...
        if rng and len(rng) > 1:
            ...
            for range_str in rng:
                range_strings.extend(self.extract_date_strings_inner(range_str[0], range_str[1][0], strict=strict))
        ...
        for match in items:
            ...
            if strict:
                complete = False
                ## 12-05-2015
                if len(digits) == 3 or ((len(months) == 1) and (len(digits) == 2)):
                    complete = True
    ...
    @classmethod
    def strip_string_entry(
        cls, text: str, text_coords: tuple[int, int], strip_chars: str | None = None
    ) -> tuple[str, tuple[int, int]]:
        ln = len(text)
        text = text.rstrip(strip_chars)
        text_coords = (
            text_coords[0],
            text_coords[1] - ln + len(text),
        )

        ln = len(text)
        text = text.lstrip(strip_chars)
        text_coords = (
            text_coords[0] + ln - len(text),
            text_coords[1],
        )
