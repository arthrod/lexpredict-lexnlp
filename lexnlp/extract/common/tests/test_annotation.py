...
class TestAnnotation(TestCase):
    ...
    def test_repr(self):
        ant = CourtAnnotation(name="GrossCourt", coords=(1, 10), text="Court Gross", locale="EN")
        s = ant.__repr__()
    ...
    def test_act_annotation(self):
        ...
        ant = ActAnnotation(
            coords=(20, 92), act_name="Some act", ambiguous=False, text="Some act of 2021, p IV", locale="pg"
        )
        cite = ant.get_cite()
    ...
    def test_date_annotation(self):
        ant = DateAnnotation(coords=(2, 12), date=date(2018, 1, 13), score=0.5, locale="pg")
        self.assertEqual("pg", ant.locale)
        s = ant.__repr__()
        self.assertGreater(len(s), 0)
    ...
    def test_condition_annotation(self):
        ant = ConditionAnnotation(coords=(1, 41), condition="in case of", pre="murder", post='dial "M"')
    ...
    def test_constraint_annotation(self):
        ant = ConstraintAnnotation(coords=(1, 41), constraint="when you are dead", post="never drive a car")
        self.assertEqual("en", ant.locale)
        s = ant.__repr__()
        self.assertGreater(len(s), 0)
    ...
    def test_cusip_annotation(self):
        ant = CusipAnnotation(coords=(2, 20), code="SBN11", tba="tba1", ppn="ppn1", internal=True, checksum="0x0A")
        self.assertEqual("en", ant.locale)
        cite = ant.get_cite()
    ...
    def test_money_annotation(self):
        ant = MoneyAnnotation(coords=(2, 20), amount=1001, locale="de", currency="JPY")
        self.assertEqual("de", ant.locale)
        cite = ant.get_cite()
        self.assertEqual("/de/money/1001/JPY", cite)

    def test_percent_annotation(self):
        ant = PercentAnnotation(coords=(2, 20), amount=101, sign="percento")
        self.assertEqual("en", ant.locale)
        cite = ant.get_cite()
