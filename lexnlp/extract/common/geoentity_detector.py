...
class GeoEntityLocator:
    ...
    def __init__(
        ...
        self.conflict_resolving_func = (
            conflicts_take_first_by_id
            if conflict_resolving_field == "id"
            else conflicts_top_by_priority
            if conflict_resolving_field == "priority"
            else None
        )
    ...
    def get_geoentity_entries(self, text: str) -> Generator[tuple[DictionaryEntry, DictionaryEntryAlias], Any, Any]:
        """
    ...
    def get_geoentity_annotations(self, text: str) -> Generator[GeoAnnotation]:
        """
        This method uses general searching routines for dictionary entities from dict_entities.py module.
        Methods of dict_entities module can be used for comfortable creating the config: entity_config(),
        entity_alias(), add_aliases_to_entity().
