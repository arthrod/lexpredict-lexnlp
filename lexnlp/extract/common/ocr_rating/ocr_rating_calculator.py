...
class BaseOcrRatingCalculator:
    ...
    def init_language_data(self, data_folders: list[str], language_file_paths: list[str] | None = None) -> None:
        """
...
class CosineSimilarityOcrRatingCalculator(BaseOcrRatingCalculator):
    ...
    def get_rating(self, text: str, language: str) -> float:
        ...
        ocr_grade = numpy.round(int(cs * 100) / 10.0)
        return ocr_grade
...
class QuadraticCosineSimilarityOcrRatingCalculator(CosineSimilarityOcrRatingCalculator):
    def get_rating(self, text: str, language: str) -> float:
        ...
        x = cs * 100  # 0.0 ... 100.0
        ocr_grade = 0 if x < 50 else numpy.round(0.00219033 * (x**2) - 0.1239173 * x + 1.1268522)
        ocr_grade = min(ocr_grade, 10)
        return ocr_grade


def build_cs_rating_calculator(
    language_file_paths: list[str] | None = None, primary_language_folder: str | None = None
) -> CosineSimilarityOcrRatingCalculator:
    # noinspection PyTypeChecker
    return build_rating_calculator(CosineSimilarityOcrRatingCalculator, language_file_paths, primary_language_folder)


def build_cs_quad_rating_calculator(
    language_file_paths: list[str] | None = None, primary_language_folder: str | None = None
) -> QuadraticCosineSimilarityOcrRatingCalculator:
    # noinspection PyTypeChecker
    return build_rating_calculator(
        QuadraticCosineSimilarityOcrRatingCalculator, language_file_paths, primary_language_folder
    )
