...
class RegulationsParser:
    """
    Parses Portuguese (Brazilian) regulations (acts, institutions and so on):
    - "a emissão de instrumentos inscritos no Registro Nacional de Valores, colocados"
      boils down to 'Registro Nacional de Valores'

    - expects words like 'lei', 'decreto', 'medida provisória', 'resolução'
    ...
    def load_trigger_words(self) -> None:
        """
        Load records like ('lei', 'start') - (trigger_word, position)
        """
