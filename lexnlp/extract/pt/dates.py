"""
Date extraction for Portuguese.
...
PT_MONTHS = sorted([y.lower() for k, v in info.items() if k in months for y in v],
...
class PtDateParser(DateParser):
    ...
    SEQUENTIAL_DATES_RE = re.compile(
        r'(?P<text>(?P<day>\d{{1,2}}) de (?P<month>{pt_months})(?:, | e | de (?P<year>\d{{4}})))'.format(
            pt_months='|'.join(PT_MONTHS)), re.I | re.M)
    WEIRD_DATES_NORM = [
        (re.compile(r'(\d+º\s?de (?:{pt_months})(?: de \d{{4}})?)'.format(
            pt_months='|'.join(PT_MONTHS)), re.I | re.M),
