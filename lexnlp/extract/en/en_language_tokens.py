...
class EnLanguageTokens:
    ...
    def init():
        ...
        if os.path.isfile(abr_file_path):
            ...
            EnLanguageTokens.abbreviations = EnLanguageTokens.abbreviations.union(file_set)

        prn_file_path = os.path.join(os.path.dirname(__file__), "data/pronouns.txt")
        if os.path.isfile(prn_file_path):
            file_set = LanguageDictionaryReader.read_str_set(prn_file_path)
            EnLanguageTokens.pronouns = EnLanguageTokens.pronouns.union(file_set)
