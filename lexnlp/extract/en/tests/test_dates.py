...
from lexnlp.extract.en.dates import get_date_features, get_dates_list, get_raw_date_list, train_default_model
...
class TestDates(TestCase):
    def test_get_raw_dates(self):
        text = "1990-2-2"
        dates = get_raw_date_list(text, strict=True, base_date=datetime.datetime(2019, 1, 1), return_source=True)
    ...
    def test_fixed_raw_dates(self):
        ...
        lexnlp_tests.test_extraction_func_on_test_data(
            get_raw_date_list, expected_data_converter=expected_data_converter
        )
    ...
    def test_fixed_dates_nonstrict(self):
        ...
        """
        lexnlp_tests.test_extraction_func_on_test_data(
            get_dates_list, strict=False, expected_data_converter=expected_data_converter
        )
    ...
    def test_fixed_date_set(self):
        date_src = [(1990, 2, 2), (2017, 1, 1), (1836, 12, 23), (2019, 11, 30), (2101, 1, 14)]
        self.check_dates_set(date_src)

    def test_random_date_set(self):
        date_src = [(random.randint(1950, 2040), random.randint(1, 12), random.randint(1, 31)) for _ in range(100)]
        self.check_dates_set(date_src)
    ...
    def test_date_feature_1_bigram(self):
        ...
        date_feature = lexnlp_tests.benchmark_extraction_func(
            get_date_features, "2000-02-02", start_index=0, end_index=10, include_bigrams=True, characters=string.digits
        )
        self.assertDictEqual(
            date_feature,
            {
