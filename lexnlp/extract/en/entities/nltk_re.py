...
def get_company_type_pipe(company_type_list: None | list[str] | dict[str, CompanyDescriptor] = None) -> str:
    ...
    company_type_pipe = "|".join([re.escape(c.lower()) for c in company_type_list])
...
def get_company_description_pipe(company_description_list: list[str] | None = None) -> str:
    ...
    company_description_pipe = "|".join([re.escape(c.strip(".").lower()) for c in company_description_list])
...
ARTICLE_RE = re.compile(ARTICLES, re.IGNORECASE | re.MULTILINE | re.UNICODE | re.DOTALL | re.VERBOSE)
ARTICLE_PATTERN = rf"""
(?:
    (?:\W|^)
    (?P<article>{ARTICLES})
...
def create_company_pattern(
    company_pattern_template=None,
    ...
    return company_pattern_template.format(
...
DEFAULT_COMPANY_DESC_RE = re.compile(r"(?:^|\W)(?:{})(?:$|\W)".format("|".join(COMPANY_DESCRIPTIONS)), re.I)
