...
def test_stanford_name_example_in():
    ...
    lexnlp_tests.test_extraction_func_on_test_data(
        get_persons, expected_data_converter=lambda row: row[0], test_only_expected_in=True
    )
...
def test_stanford_org_example_in():
    ...
    lexnlp_tests.test_extraction_func_on_test_data(
        get_organizations, expected_data_converter=lambda row: row[0], test_only_expected_in=True
    )
