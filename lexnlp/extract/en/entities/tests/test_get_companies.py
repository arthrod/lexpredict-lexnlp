...
class TestGetCompanies(TestCase):
    def test_get_unpreffixed_companies(self):
        texts = [
            "MI 48226 From: Company City of Detroit, Contact ROMONA JONES Address COLEMAN",
        ...
        all_companies = []
        for text in texts:
            comps = list(get_company_annotations(text, banlist_usage=BanListUsage(use_default_banlist=False)))
            all_companies += comps
        self.assertEqual(1, len(all_companies))

    def test_copyright(self):
        text = (
            "Copyright (c) 2019, Moody's Corporation, Moody's Investors Service, Inc., "
...
The Repository Trust and Clearing Corporation ("RTCC")."""
    ...
    def test_custom_banlisted(self):
        text = (
            'Depository Bank is a partially-owned subsidiary of The Depository Trust and Cleaning Corporation ("DTCC").'
        )
        custom_bl = [EntityBanListItem("Cleaning")]
        comps = list(
            get_company_annotations(text, banlist_usage=BanListUsage(banlist=custom_bl, append_to_default=True))
        )
        self.assertEqual(1, len(comps))

        comps = list(
            get_company_annotations(
                text, banlist_usage=BanListUsage(banlist=custom_bl, use_default_banlist=False, append_to_default=False)
            )
        )
        self.assertEqual(len(comps), 2)
    ...
    def test_reg_back(self):
        ...
        text = (
            "/NOR <FEFF004200720075006b002000640069007300730065002000"
