...
class TestPhraseConstructor(TestCase):
    def test_join_class_nonstrict_01(self):
        ...
        phrases = list(PhraseConstructor.join_tokens_by_class(tokens, predicted, strict=False))
    ...
    def test_join_class_strict_01(self):
        tokens = [(i * 10, i * 10 + 9) for i in range(8)]
        predicted = np.array([0, 0, 1, 1, 2, 2, 0, 0])
        phrases = list(PhraseConstructor.join_tokens_by_class(tokens, predicted, strict=True))
    ...
    def test_join_score_1_01(self):
        tokens = [(i * 10, i * 10 + 9) for i in range(8)]
        predicted = np.array([0, 0, 1, 1, 2, 2, 0, 0])
        phrases = list(PhraseConstructor.join_tokens_by_score(tokens, predicted, max_zeros=1, min_token_score=1))
        self.assertEqual(2, len(phrases))
    ...
    def test_join_score_2_01(self):
        tokens = [(i * 10, i * 10 + 9) for i in range(8)]
        predicted = np.array([0, 0, 1, 1, 2, 2, 0, 0])
        phrases = list(PhraseConstructor.join_tokens_by_score(tokens, predicted, max_zeros=1, min_token_score=2))
        self.assertEqual(1, len(phrases))
    ...
    def test_join_score_2_02(self):
        ...
        phrases = list(PhraseConstructor.join_tokens_by_score(tokens, predicted, max_zeros=2, min_token_score=2))
        self.assertEqual(1, len(phrases))
