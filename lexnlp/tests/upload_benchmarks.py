...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        dest="csv_file",
        type=str,
        default=lexnlp_tests.FN_BENCHMARKS,
        help="Path/name of csv file with benchmarks",
    )
    parser.add_argument("--es-url", dest="url", type=str, default="localhost:9200", help="ElasticSearch host:port/path")
    parser.add_argument("--es-username", dest="username", type=str, default=None, help="ElasticSearch Username")
    parser.add_argument("--es-password", dest="password", type=str, default=None, help="ElasticSearch Password")
    parser.add_argument("--es-use-ssl", dest="use_ssl", type=bool, default=False, help="ElasticSearch use SSL")
    parser.add_argument(
        "--es-verify-certs", dest="verify_certs", type=bool, default=True, help="ElasticSearch verify_certs"
    )
    parser.add_argument(
        "--es-index-prefix", dest="index_prefix", type=str, default="benchmarks", help="ElasticSearch index prefix"
    )
...
if __name__ == "__main__":
    ...
    processed = process_data(
        cmd_args.csv_file, index=index_name, process_func=lambda actions: helpers.bulk(es, actions)
    )
