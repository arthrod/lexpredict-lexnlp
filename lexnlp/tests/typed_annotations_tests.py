...
class TypedFieldCheck:
    ...
    def parse(line: str):
        ...
        return TypedFieldCheck(index=index, path=path_parts, value=val, comparison=comp_sign, check_all=check_all)
...
class TypedAnnotationsTester:
    ...
    def test_and_raise_errors(self, parsing_method: Callable, file_name: str, entity_type: type):
    ...
    def test_parser(self, parsing_method: Callable, file_name: str, entity_type: type):
    ...
    def process_cases(self) -> tuple[int, int]:
        for case_index in range(len(self.test_cases)):
            ...
            try:
                ...
                self.errors.append(ParsingError(message=f"Exception while parsing {case_index} case.", exception=e))
                continue
    ...
    def check_case_annotations(self, ants: list[TextAnnotation], case: TypedAntTestCase, case_index: int):
        count = len(ants)
        if count != case.total:
            self.errors.append(ParsingError(message=f"Read {count} annotations instead of {case.total}:\n{case}"))
    ...
    def process_one_check(self, ant: TextAnnotation, check: TypedFieldCheck, case_index: int, check_index: int) -> None:
        try:
        ...
        if equals:
            return
        if not cmp_ok:
            self.errors.append(ParsingError(message=check.last_error, case_index=case_index, check_index=check_index))
            return
